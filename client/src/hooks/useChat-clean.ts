...
import { streamChat, fetchSuggestions } from '@/lib/api-clean';

type Msg = { role: 'user' | 'assistant'; text: string };

export function useChat(init: { 
  major: string; 
  subField: string; 
  suggestCount?: number; 
  ...
  const [conversationId, setCid] = useState(() => {
    const saved = typeof window !== 'undefined' ? localStorage.getItem('cid') : null;
    const cid = init.conversationId ?? saved ?? crypto.randomUUID();
    if (!saved && typeof window !== 'undefined') localStorage.setItem('cid', cid);
    return cid;
  });
